...
# lkp edit start 
# custom module for emulating grouped convolution 
class SplitConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=(1,1), padding=(0,0), use_deconv=False, is_last=False):
        super().__init__()
        self.use_deconv = use_deconv
        
        # We cut the channels exactly in half to replicate groups=2
        half_in = in_channels // 2
        half_out = out_channels // 2
        
        if use_deconv:
            # --- DECODER MODE (Expanding) ---
            # Use 1x1 convolutions to mix channels safely
            self.conv_a = nn.Conv2d(half_in, half_out, kernel_size=1)
            self.conv_b = nn.Conv2d(half_in, half_out, kernel_size=1)
            
            # Save original params for the math formula later
            self.k = kernel_size if isinstance(kernel_size, tuple) else (kernel_size, kernel_size)
            self.s = stride if isinstance(stride, tuple) else (stride, stride)
            self.p = padding if isinstance(padding, tuple) else (padding, padding)
        else:
            # --- ENCODER MODE (Shrinking) ---
            # The compiler safely handles standard Conv2d with strides
            self.conv_a = nn.Conv2d(half_in, half_out, kernel_size, stride, padding)
            self.conv_b = nn.Conv2d(half_in, half_out, kernel_size, stride, padding)
    ...
    def forward(self, x):
        # 1. SPLIT the tensor perfectly in half
        part_a, part_b = torch.chunk(x, 2, dim=1)
        
        # 2. CONVOLVE independently
        out_a = self.conv_a(part_a)
        out_b = self.conv_b(part_b)
        
        # 3. GLUE back together
        x_mixed = torch.cat([out_a, out_b], dim=1)
        
        # 4. UPSAMPLE (Only if it's a Decoder)
        if self.use_deconv:
            H_out = (x_mixed.shape[2] - 1) * self.s[0] - 2 * self.p[0] + self.k[0]
            W_out = (x_mixed.shape[3] - 1) * self.s[1] - 2 * self.p[1] + self.k[1]
            x_mixed = torch.nn.functional.interpolate(x_mixed, size=(H_out, W_out), mode='nearest')
            
        # 5. Apply Batchnorm and Activation
        return self.act(self.bn(x_mixed))    
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups=1, use_deconv=False, is_last=False):
        ...
        self.use_deconv = use_deconv
        if use_deconv:
            # Only apply the upsample proxy in the Decoder
            ...
            # Save the parameters so we can use them in our math formula later
            self.k = kernel_size if isinstance(kernel_size, tuple) else (kernel_size, kernel_size)
            self.s = stride if isinstance(stride, tuple) else (stride, stride)
            self.p = padding if isinstance(padding, tuple) else (padding, padding)
        else:
            # NORMAL CONV: Let the Encoder shrink the audio naturally
    ...
    def forward(self, x):
        ...
        if self.use_deconv:
            ...
            # THE MAGIC: PyTorch's exact ConvTranspose2d mathematical output formula
            # L_out = (L_in - 1) * stride - 2 * padding + kernel_size
            H_out = (x.shape[2] - 1) * self.s[0] - 2 * self.p[0] + self.k[0]
            W_out = (x.shape[3] - 1) * self.s[1] - 2 * self.p[1] + self.k[1]
            
            # Force the tensor to be exactly the computed size (e.g., 65 instead of 66)
            x = torch.nn.functional.interpolate(x, size=(H_out, W_out), mode='nearest')
...
class StreamEncoder(nn.Module):
    def __init__(self):
        ...
        self.en_convs = nn.ModuleList([
            ...
            # ConvBlock(16, 16, (1,5), stride=(1,2), padding=(0,2), groups=1, use_deconv=False, is_last=False), #modified by lkp ; groups=2 to 1
            SplitConvBlock(16, 16, (1,5), stride=(1,2), padding=(0,2), use_deconv=False, is_last=False), #lkp edit: custom func instead of groups=2 conv
    ...
    #lkp edit start: make cache slicing external to model
    ...
    def forward(self, x, c_en1, c_en2, c_en3, t_en1, t_en2, t_en3):
        """
        Inputs are now discrete cache chunks.
        c_en1 replaces conv_cache[:,:, :2, :]
        c_en2 replaces conv_cache[:,:, 2:6, :]
        c_en3 replaces conv_cache[:,:, 6:16, :]
        """
        en_outs = []
        for i in range(2):
            x = self.en_convs[i](x)
        ...
        #no slicing happens here now
        x, out_c_en1, out_t_en1 = self.en_convs[2](x, c_en1, t_en1); en_outs.append(x)
        x, out_c_en2, out_t_en2 = self.en_convs[3](x, c_en2, t_en2); en_outs.append(x)
        x, out_c_en3, out_t_en3 = self.en_convs[4](x, c_en3, t_en3); en_outs.append(x)
        
        out_c_caches = (out_c_en1, out_c_en2, out_c_en3)
        out_t_caches = (out_t_en1, out_t_en2, out_t_en3)

        return x, en_outs, out_c_caches, out_t_caches
...
#             # ConvBlock(16, 16, (1,5), stride=(1,2), padding=(0,2), groups=1, use_deconv=True, is_last=False),#modified by lkp ; groups=2 to 1
#             SplitConvBlock(16, 16, (1,5), stride=(1,2), padding=(0,2), use_deconv=True, is_last=False), #lkp edit: custom func instead of groups=2 conv
...
class StreamDecoder(nn.Module):
    ...
    def forward(self, x, en_outs, c_de1, c_de2, c_de3, t_de1, t_de2, t_de3):
        """
        Inputs are now discrete cache chunks.
        c_de1 replaces conv_cache[:,:, 6:16, :]
        c_de2 replaces conv_cache[:,:, 2:6, :]
        c_de3 replaces conv_cache[:,:, :2, :]
        """

        x, out_c_de1, out_t_de1 = self.de_convs[0](x + en_outs[4], c_de1, t_de1)
        x, out_c_de2, out_t_de2 = self.de_convs[1](x + en_outs[3], c_de2, t_de2)
        x, out_c_de3, out_t_de3 = self.de_convs[2](x + en_outs[2], c_de3, t_de3)
        
        for i in range(3, 5):
            x = self.de_convs[i](x + en_outs[4-i])
            
        out_c_caches = (out_c_de1, out_c_de2, out_c_de3)
        out_t_caches = (out_t_de1, out_t_de2, out_t_de3)
            
        return x, out_c_caches, out_t_caches
#lkp edit end
...
class StreamGTCRN(nn.Module):
    def __init__(self):
        ...
        self.mask = Mask()
    #lkp edit: make cache slicing external to model
    ...
    def forward(self, spec, 
                    c_en1, c_en2, c_en3, t_en1, t_en2, t_en3,  # 6 Encoder Caches
                    i_rnn1, i_rnn2,                            # 2 RNN/TCN Caches
                    c_de1, c_de2, c_de3, t_de1, t_de2, t_de3   # 6 Decoder Caches
                ):
            ...
            # 2. ENCODER: Pass the 6 discrete chunks in. Get the 6 updated chunks out.
            feat, en_outs, out_c_en, out_t_en = self.encoder(feat, c_en1, c_en2, c_en3, t_en1, t_en2, t_en3)

            # 3. RNN: Pass their discrete chunks
            feat, out_i_rnn1 = self.dpgrnn1(feat, i_rnn1) 
            feat, out_i_rnn2 = self.dpgrnn2(feat, i_rnn2) 

            # 4. DECODER: Pass the 6 discrete chunks in.
            m_feat, out_c_de, out_t_de = self.decoder(feat, en_outs, c_de1, c_de2, c_de3, t_de1, t_de2, t_de3)
            ...
            # RETURN EVERYTHING FLAT: TFLite prefers flat lists of outputs
            return spec_enh, out_c_en, out_t_en, out_i_rnn1, out_i_rnn2, out_c_de, out_t_de
